import { OPERATION_ID, OPERATION_PROPS, OPERATION_RESULT } from '../../../constants/echo.constants';
...
type OP_ID = OPERATION_ID.CONTRACT_INTERNAL_CREATE;

export default class ContractInternalCreateOperaiton extends AbstractOperation<OP_ID> {
	id: OPERATION_ID = OPERATION_ID.CONTRACT_INTERNAL_CREATE;
	...
	public async parse(
		body: OPERATION_PROPS<OP_ID>,
		result: OPERATION_RESULT<OP_ID>,
		dBlock: TDoc<IBlock>,
		...
		const code = await this.echoRepository.getContract(body.new_contract).then((res) => res[1].code);
